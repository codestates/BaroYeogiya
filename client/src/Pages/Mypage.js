...
export default function Mypage({
  ...
  if (!withDrawalModal && !modifyModal && !cartlistModal) {
    return (
      <>
        <button className="cartlist-button" onClick={() => { handleCartlistModal(); likeStore(); } }>
          찜 목록
        </button>
        <UserInfo
          userInfo={userInfo}
    ...
    return (
      <>
        <div className='cart-list-view'>
          <button className='my-list-bt' onClick={handleCartlistModal}>찜 목록</button>
          <div className='store-card-container'>
            { myList.map((like) => <CartList like={like} userInfo={userInfo} /> ) }
          </div>
        </div>
      </>
    );
